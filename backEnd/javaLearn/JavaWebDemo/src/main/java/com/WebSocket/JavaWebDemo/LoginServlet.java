 ...
 **/
@WebServlet(value = "/websocketlogin")
...
//    ResultSet rs = stmt.executeQuery(sql);
    ...
    private String signup(String username, String passwd) {
        ResultSet temp = null;
        try {
            temp = stmt.executeQuery("select passwd from UserForWS where user = '" + username + "'");
        ...
        try {
            if (temp.next()) {
                ...
                stmt.execute("INSERT INTO UserForWS VALUES ('" + username + "','" + passwd + "')");
                return "注册成功！";
            }
    ...
    private String login(String username, String passwd) {
        ResultSet pass = null;
        try {
            pass = stmt.executeQuery("select passwd from UserForWS where user = '" + username + "'");
            if (pass.next()) {
