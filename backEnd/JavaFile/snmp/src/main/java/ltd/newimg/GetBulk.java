...
public class GetBulk {
    public static void main(String[] args) throws IOException {
        ...
        // 初始化并设置PDU
        PDU pdu = new PDU() {{
            add(new VariableBinding(new OID(new int[]{1, 3, 6, 1, 2, 1, 4, 3, 0})));
            add(new VariableBinding(new OID(new int[]{1, 3, 6, 1, 2, 1, 4, 9, 0})));
            add(new VariableBinding(new OID(new int[]{1, 3, 6, 1, 2, 1, 4, 15, 0})));
            setNonRepeaters(1);
            setMaxRepetitions(4);
            setType(PDU.GETBULK);
        }};
